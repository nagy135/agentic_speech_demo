...
### Scheduled sideband messages

Every active session receives a server-side nudge every 30 seconds, starting 30 seconds after `session.started`. The default message is **“tell me current time”**, which the server expands into the current **Europe/Berlin** time (including seconds and automatic daylight-saving adjustment) and a request to announce it in the conversation's language. The server sends `session.commentary.append` with `delegation_id: null` and a unique `time-nudge-…` event ID. The model controls when the speech actually happens; 30 seconds is the send interval, not an exact playback schedule.

During a chat, open **Debug**, edit **Message sent every 30 seconds**, and click **Save**. Save posts to `/api/session/nudge` using a private per-session token. The server stores the message in that session's in-memory timer state and uses it on the next scheduled tick without resetting the timer or sending immediately. Custom messages replace the time announcement and are sent as written (surrounding whitespace is trimmed). Blank messages and messages over 400 UTF-8 bytes are rejected. Save errors leave the previous confirmed message active.

The browser has no repeating sender and does not persist this setting in browser storage. Messages are isolated between conversations and cleared when their sideband closes; each new chat starts with the default. The token stays in the browser transport and is redacted from debug exports. The session registry is shared across route bundles in the single Node process; multiple server replicas would require routing to the session-owning process or a shared control channel.
